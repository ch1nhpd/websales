    ...
    <?php
        if (isset($_GET['dathang']) && $_GET['dathang'] == 'true') {
            for ($i = 0; $i < $count; $i++) {		
                ...
                $sql = "INSERT INTO chitietdathang VALUES ('$MSDH', (SELECT MSHH FROM hanghoa WHERE TenHH = '$arrTenHH[$i]'), $arrValue[$i], $arrTongTien[$i]);";
                $sql .="INSERT INTO dathang VALUES ('$MSDH', (SELECT MSKH FROM khachhang WHERE TaiKhoan = '$_SESSION[dangnhap]'), NULL, '$date', 'false');";
                $sql .="UPDATE hanghoa SET SoLuongHang = SoLuongHang - $arrValue[$i] WHERE MSHH = (SELECT MSHH FROM hanghoa WHERE TenHH = '$arrTenHH[$i]');";
